...
////<ExcelBuddyConfig>BuddyExcel = WS-NF_PXI_TCF_Rev1.0.xlsx;ExcelDisplay = 1<ExcelBuddyConfig/>
...
public class TestPlan : MarshalByRefObject, IATFTest
{
    ...
    public string DoATFInit(string args)
    {
        ...
        if (System.IO.File.Exists(Cassette_Wafer_List))
        {
            ...
            string StdChk = myUtility.ReadTextFile(Cassette_Wafer_List, "PROBE", "STDCHK").ToUpper().Trim();
            b_Std_Chk = bool.Parse(StdChk);
        }
        else
        {
            if (!b_Std_Chk)
            {
                if (MessageBox.Show("Perform Standard Check Probing ?", "Notice", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk) == DialogResult.Yes)
                {
                    b_Std_Chk = true;
                }
            }
        ...
        try
        {
            TestSite = int.Parse(myUtility.ReadTextFile(Cassette_Wafer_List, "PROBE", "CORETYPE"));
        }
        catch
        {
            TestSite = 1;
        ...
        string ProbeAddress = ""; //Not Required, Parse from Handler Plugin
        ...
        if (b_Std_Chk)
        {
            try
            {
                if (System.IO.File.Exists(myUtility.ReadTextFile(Local_Setting_File, "SERVER", "FILESERVER").TrimEnd('\\') + "\\Avago.pCD\\Shared-Third\\" + myUtility.ReadTextFile(Cassette_Wafer_List, "GENERAL", "Product") + "_v" + myUtility.ReadTextFile(Cassette_Wafer_List, "PCD", "VERSION_TEXT") + "\\" + Avago.ATF.StandardLibrary.ATFCrossDomainWrapper.Excel_Get_Input("Common", 55, 2)))
                {
                    SpecLimitFileName_2ndPass = myUtility.ReadTextFile(Local_Setting_File, "SERVER", "FILESERVER").TrimEnd('\\') + "\\Avago.pCD\\Shared-Third\\" + myUtility.ReadTextFile(Cassette_Wafer_List, "GENERAL", "Product") + "_v" + myUtility.ReadTextFile(Cassette_Wafer_List, "PCD", "VERSION_TEXT") + "\\" + Avago.ATF.StandardLibrary.ATFCrossDomainWrapper.Excel_Get_Input("Common", 55, 2);
                }
                else
                {
                    SpecLimitFileName_2ndPass = myUtility.ReadTextFile(Local_Setting_File, "SERVER", "FILESERVER").TrimEnd('\\') + "\\" + myUtility.ReadTextFile(Local_Setting_File, "SERVER", "STDCHK").Trim('\\') + "\\" + Avago.ATF.StandardLibrary.ATFCrossDomainWrapper.Excel_Get_Input("Common", 55, 2);
                }
            }
            catch
            {
        ...
        try
        {
            ...
            SP.AppendFormat("{0};", "");                            //0 - Prober Address - Not Required Set from Handler Plug In
            ...
            SP.AppendFormat("{0};","");   //39 Ignore 1 Pass Spec
        ...
        string PCB_ID_Str = "123456";
        //string PCB_ID_Str = myUtility.ReadTextFile(WaferList_File, "GENERAL", "RunCard").Trim() + ";"
        //                    + myUtility.ReadTextFile(WaferList_File, "GENERAL", "Cassette").Trim() + ";"
        //                    + myUtility.ReadTextFile(WaferList_File, "GENERAL", "Setup").Trim();
        ...
        ATFCrossDomainWrapper.StoreStringToCache(PublishTags.PUBTAG_WAFER_ID, "ABCDEF");
        #endregion
            ...
            if (b_Retest)
            {
                ...
                else
                {
                    ...
                    if (!b_AMapSetting)
                    {
                        ...
                        SP1.AppendFormat("{0};", WaferID);
                        SP1.AppendFormat("{0};", ProbeID);
                        SP1.AppendFormat("{0};", ProductTag);
                        SP1.AppendFormat("{0};", TesterID);
                        SP1.AppendFormat("{0};", b_AutoAMapGenerate.ToString());
                        SP1.AppendFormat("{0};", cc.CurResultFileName);
                        //SP1.AppendFormat("{0};", A_Map.ToString());
                        if (A_Map)
                        {
                            SP1.AppendFormat("{0};", "true");
                        }
                        else
                        {
                            SP1.AppendFormat("{0};", "false");
                        }
                        //SendMsg(WaferID + ProbeID+ProductTag+TesterID+b_AMapSetting.ToString());


                        SP1.AppendFormat("{0};", "CHIPBOND");
                        SP1.AppendFormat("{0};", "FBAR");
                        SP1.AppendFormat("{0};", "FILTER");
                        SP1.AppendFormat("{0};", ProductTag);
                        SP1.AppendFormat("{0};", WaferID);
                        SP1.AppendFormat("{0};", ATFCrossDomainWrapper.GetStringFromCache(PublishTags.PUBTAG_OP_ID, ""));
                        SP1.AppendFormat("{0};", TesterID);
                        SP1.AppendFormat("{0};", "");
                        SP1.AppendFormat("{0};", "");
                        SP1.AppendFormat("{0};", ProductTag);
                        SP1.AppendFormat("{0};", "C:\\Avago.ATF.Common\\Clotho_SDI_Data\\FBAR_" + Modify_output_filename(Remove_Ext(ATFCrossDomainWrapper.GetStringFromCache(PublishTags.PUBTAG_CUR_RESULT_FILE, ""))) + "\\");

                        //SP1.AppendFormat("{0};", Drivers[0].Get_TotalChannel());
                        SP1.AppendFormat("{0};", "");
